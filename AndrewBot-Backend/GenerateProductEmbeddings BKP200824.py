...
def generate_input_text_for_product_embedding(
    ...
    input_text = (
        f"{productName_cleaned} {brandName_cleaned} {mainCategory_cleaned} "
      + f"{subCategory1_cleaned} {subCategory2_cleaned}"
    )
...
def open_pickle_files():
    ...
    product_embeddings_file_obj1 = open(product_embeddings_file_path1, "ab")
...
def close_pickle_files(
    ...
    product_embeddings_file_obj1.close()
...
def process_product_names(row, model, product_embeddings_file_obj1):
    ...
    input_text = (
        f"productName: {productName_cleaned}. " 
        f"brandName: {brandName_cleaned}. " 
        f"Category: {mainCategory_cleaned} {subCategory1_cleaned} {subCategory2_cleaned}."
    )
    ...
    status = write_embeddings(
        product_embedding_list, product_embeddings_file_obj1
    )
...
def main():
    ...
    for index, row in products_inventory_df.iterrows():
        ...
        status = process_product_names(row, model, product_embeddings_file_obj1)
