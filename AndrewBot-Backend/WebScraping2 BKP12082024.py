...
BASE_URL = "https://www.bigbasket.com/listing-svc/v2/products?type=pc&slug=foodgrains-oil-masala"
...
HEADERS_PRODUCT = {
    ...
    "Referer": "https://www.bigbasket.com",
    "X-Requested-With": "XMLHttpRequest",
    "Cookie": 'x-entry-context-id=100; x-entry-context=bb-b2c; _bb_locSrc=default; x-channel=web; _bb_loid=j:null; _bb_bhid=; _bb_nhid=1723; _bb_vid=MTI3MTYyMjAwNjE=; _bb_dsevid=; _bb_dsid=; csrftoken=8oxjDnGDIhO9WMm3I8GyzVlj8as0SPmwTHtIqqr2LZlE6Ch5WnS3ExUFUltJGS69; _bb_home_cache=dd66856c.1.visitor; bb2_enabled=true; ufi=1; bigbasket.com=0591c50b-5f4f-4650-8ac4-fba1f789fbe3; _gcl_au=1.1.1389406266.1720768559; jarvis-id=82654d75-d81f-4b8e-b147-77eaef4e66b5; _bb_tc=0; _bb_rdt="MzEwNDYxMDgwMw==.0"; _bb_rd=6; fs_uid=#11EGJ5#6039797689561088:9204439023068453163:::#/1752304567; adb=0; _gid=GA1.2.1009321565.1723119188; _client_version=2789; _bb_lat_long=MTIuOTQ5MTU4OTA3MXw3Ny41NzQwMjI2NzMx; _bb_hid=7427; BBAUTHTOKEN=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..4VTofu9_B9xKlDpGwHSpOtnhTEYqF1zi64j0YBWucyY; _bb_mid="MzA5Mjc0NzQ5NA=="; sessionid=4k6fwk0n9u845sc95yomlrltxu51yx1w; _bb_bb2.0=1; is_global=0; _bb_addressinfo=; _bb_pin_code=; _bb_sa_ids=10179; _is_bb1.0_supported=0; _bb_cda_sa_info=djIuY2RhX3NhLjEwMC4xMDE3OQ==; is_integrated_sa=0; _bb_visaddr=; _gcl_gs=2.1.k1$i1723182717; _gac_UA-27455376-1=1.1723182724.CjwKCAjw2dG1BhB4EiwA998cqFGK4WRIFqgCFwBUzpEv9LVOJoVx9VLhjUKaPcmuhVxkCcb8tq2bdBoCDhMQAvD_BwE; _gcl_aw=GCL.1723182724.CjwKCAjw2dG1BhB4EiwA998cqFGK4WRIFqgCFwBUzpEv9LVOJoVx9VLhjUKaPcmuhVxkCcb8tq2bdBoCDhMQAvD_BwE; _bb_cid=1; _bb_aid="Mjg3MTIxMjQ3NQ=="; csurftoken=oY64QA.MTI3MTYyMjAwNjE=.1723354962367.eZYfSXRA3JGYd0ExHntJaA5cDKRthWIpIaZWNYIUQSw=; _is_tobacco_enabled=0; ts="2024-08-11 11:16:18.316"; _ga_FRRYG5VKHX=GS1.1.1723355172.18.1.1723355178.54.0.0; _ga=GA1.2.1996715233.1720768560; _gat_UA-27455376-1=1',
}
...
def parse_products(json_data):
    ...
    for tab in tabs:
        ...
        for product in product_list:
            ...
            all_product_details.append(extract_product_details(product, "N"))

            # Extract details for each child product
            for child in product.get("children", []):
                all_product_details.append(extract_product_details(child, "Y"))

            product_id = product.get("id")

            product_descriptions = get_product_description(product_id)

            all_product_descriptions.append(product_descriptions)

    return all_product_details, all_product_descriptions
...
def parse_product_description(product_id, json_data, main_product_flag):
    ...
    print("main product id for description extraction:", product_id)
...
def get_product_description(product_id):
    # print("within get_product_description")
        ...
        if script_tag:
            ...
            if json_data:
                product_descriptions.extend(
                    parse_product_description(product_id, json_data, main_product_flag)
                )
                # print("product_descriptions:", product_descriptions)
                return product_descriptions
            else:
...
def main():
    start_page_number = 30
    end_page_number = start_page_number + 15
    ...
    while True:
        ...
        if json_data and json_data != hold_json_data:
            product_details, all_product_descriptions = parse_products(json_data)
            ...
            if not product_details:
                ...
                if page_number == start_page_number:
                    ...
                    merged_product_details_with_description_df.to_csv(
                        "product_details.csv", index=False
                    )
                else:
                    # skip writing of header for next batch and append the dataframe to the csv.
                    merged_product_details_with_description_df.to_csv(
                        "product_details.csv", index=False, mode="a", header=False
                    )

                print(f"Data saved to product_details.csv")
                # save_to_csv(merged_product_details_with_description, "product_details.csv")

                print(
                    f"Page {page_number} - Total products retrieved so far: {len(product_details)}"
                )
                
                #To process for only 1 page of slug data, uncomment the below if block.
                # if page_number == start_page_number:
                #     break
                page_number += 1
                hold_json_data = json_data

        elif json_data and json_data == hold_json_data:
            print("NO MORE PAGE NUMBERS TO SCRAPE, ENDING SCRAPE.")
            break
        else:
