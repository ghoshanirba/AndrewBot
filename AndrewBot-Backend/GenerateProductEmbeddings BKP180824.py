...
def generate_input_text_for_embedding(row):
    ...
    input_text = (
        f"The product is named {productName} and belongs to the brand {brand}. "
        + f"It's weight is {weight} and belongs to the main category {mainCategory}, "
        + f"1st sub-category {subCategory1} and 2nd sub-category {subCategory2}."
    )
...
def main():
    ...
    for index, row in products_inventory_df.iterrows():
        ...
        input_text = generate_input_text_for_embedding(row)
