...
def destructure_order_details(order_details):
    ...
    chatGPT_response = {
        "order": [
            ...
            {"item": "chana sattu", "brand": "mana", "weight": "200", "weightUnits": "grams"},
            {
                "item": "rice flour",
                "brand": "bb royal",
                "weight": "100",
...
def process_product(
    ...
    input_text_product_name = (f"productName: {user_request_product_name_cleaned}")
    print("input_text_product_name:", input_text_product_name)
    productName_embedding = generate_embedding(input_text_product_name, model)
    user_reuest_productName_embedding_array = np.array(productName_embedding)
    ...
    input_text_brand_name = (f"brandName: {best_matched_brand_name_cleaned}")
    print("input_text_brand_name:", input_text_brand_name)
    brandName_embedding = generate_embedding(input_text_brand_name, model)
    user_reuest_brandName_embedding_array = np.array(brandName_embedding)
    # input_text = (
    #     f"{user_request_product_name_cleaned} {best_matched_brand_name_cleaned} "
    #     + f"{best_matched_brand_name_cleaned} {user_request_product_name_cleaned} "
    #     + f"{user_request_product_name_cleaned}"
    # )
    
    # input_text = (
    #     f"{user_request_product_name_cleaned} {user_request_product_name_cleaned} {user_request_product_name_cleaned} {user_request_product_name_cleaned}, "
    #   + f"{best_matched_brand_name_cleaned} {best_matched_brand_name_cleaned}."
    # )

    # input_text = input_text.lower()
    # print("input:", input_text)
    # Combine embeddings with weights
    user_request_weighted_embedding = (
              product_name_weight * user_reuest_productName_embedding_array
            + brand_name_weight * user_reuest_brandName_embedding_array
        )
    ...
    best_matched_productID = find_closest_matched_product_ID(
        user_request_weighted_embedding, product_embeddings_df
    )

    top_matched_product_ids = find_top_matched_product_ID(
        user_request_weighted_embedding, product_embeddings_df, top_n
    )

    return best_matched_productID, top_matched_product_ids


def process_product1(
    user_request_product_name,
    best_matched_brand_name,
    user_request_product_weight,
    user_request_product_weight_unit,
    model,
    product_embeddings_df1,
    top_n,
):
    allowable_chars_product_name = "&%"
    replacement_char_product_name = " "
    ...
    # input_text = (
    #     f"productName: {user_request_product_name_cleaned}. "
    #     f"brandName: {best_matched_brand_name_cleaned} {best_matched_brand_name_cleaned}. "
    #     f"Category: {best_matched_brand_name_cleaned} {user_request_product_name_cleaned} {user_request_product_name_cleaned}."
    # )
    
    input_text = (
        f"productName: {user_request_product_name_cleaned}. " 
        f"brandName: {best_matched_brand_name_cleaned} {best_matched_brand_name_cleaned} {best_matched_brand_name_cleaned}. " 
    )
    ...
    # do a look up in the Products embedding file to find the best matched product ID.
    best_matched_productID = find_closest_matched_product_ID(
        user_request_product_embedding, product_embeddings_df1
    )

    top_matched_product_ids = find_top_matched_product_ID(
        user_request_product_embedding, product_embeddings_df1, top_n
    )
...
def find_top_matched_product_ID(
    ...
    return top_matched_product_ids
...
def create_order():
    ...
    # product_embeddings_file_obj1 = open_pickle_files(product_embeddings_file_path1)

    if (
        (brand_name_embeddings_file_obj is None)
        or (product_embeddings_file_obj is None)
        # or (product_embeddings_file_obj1 is None)
    ):
        ...
        # product_embeddings_df1 = load_embeddings(product_embeddings_file_obj1)
        close_pickle_files(brand_name_embeddings_file_obj)
        close_pickle_files(product_embeddings_file_obj)
        # close_pickle_files(product_embeddings_file_obj1)
        ...
        for product in order_details_json:
            ...
            best_matched_productID, top_matched_product_ids = process_product(
                user_request_product_name,
                best_matched_brand_name,
                user_request_product_weight,
                user_request_product_weight_unit,
            ...
            print("top matched product ids:", top_matched_product_ids)
            # print(
            #     "----------------------------------------------------------------------"
            # )
            # Find best matched product ID Again by another input text
            # best_matched_productID1, top_matched_product_ids1 = process_product1(
            #     user_request_product_name,
            #     best_matched_brand_name,
            #     user_request_product_weight,
            #     user_request_product_weight_unit,
            ...
            #     top_n,
            # )
            # print("best matched productID1:", best_matched_productID1)
            # print("top matched product ids:", top_matched_product_ids1)
