...
def generate_input_text_for_product_embedding(
    ...
    input_text = (
        f"The product is named {productName_cleaned}. It belongs to the brand {brandName_cleaned}. "
      + f"It's main category is {mainCategory_cleaned}, 1st sub-category is {subCategory1_cleaned} "
      + f"and 2nd sub-category is {subCategory2_cleaned}."
    )
...
def open_pickle_files():
    ...
    product_name_embeddings_file_obj = open(product_name_embeddings_file_path, "ab")

    unique_brandName_embeddings_file_obj = open(
        unique_brandName_embeddings_file_path, "ab"
    )
    print("pickle files opened.")

    return (
        product_embeddings_file_obj,
        product_name_embeddings_file_obj,
        unique_brandName_embeddings_file_obj,
    )
...
def close_pickle_files(
    ...
    product_name_embeddings_file_obj.close()
    unique_brandName_embeddings_file_obj.close()
...
def process_product_names(row, model, product_name_embeddings_file_obj):
    ...
    input_text = generate_input_text_for_name_embedding(
        productName_cleaned,
        mainCategory_cleaned,
        subCategory1_cleaned,
        subCategory2_cleaned,
    ...
    product_name_embedding = generate_embedding(input_text, model)

    product_name_embedding_list.append(
        {"productID": productID, "nameEmbedding": product_name_embedding}
    )

    status = write_embeddings(
        product_name_embedding_list, product_name_embeddings_file_obj
    )
...
def process_products(row, model, product_embeddings_file_obj):
    ...
    status = write_embeddings(
        product_embedding_list, product_embeddings_file_obj
    )
...
def main():
    ...
    for index, row in products_inventory_df.iterrows():
        ...
        status = process_product_names(row, model, product_name_embeddings_file_obj)

        if status == "Embedding have been saved":
            total_product_names_write_counter += 1
        else:
            print("ERROR OCCURRED DURING WRITING NAME EMBEDDING PICKLE FILE")
    ...
    print("Total product embedding records written:", total_product_embeddings_write_counter)
