...
def process_product(
    ...
    #Create embeddings for user request product name
    input_text_product_name = (
        f"The product is named {user_request_product_name_cleaned}. "
        f"{user_request_product_name_cleaned} is known for its quality."
    )
    ...
    user_reuest_product_name_embedding_array = np.array(product_name_embedding).reshape(1, -1)
    
    #Create embeddings for user request brand name
    user_request_brand_name_cleaned = clean_input_string(
        user_request_brand_name, allowable_chars_brand_name, replacement_char_brand_name
    )
    
    input_text_brand_name = (
        f"The brand of this product is {user_request_brand_name_cleaned}. "
        f"{user_request_brand_name_cleaned} is a trusted and popular brand."
    )
    ...
    user_reuest_brand_name_embedding_array = np.array(brand_name_embedding).reshape(1, -1)
    
    product_embeddings_df = pd.DataFrame(product_embeddings_df)
    # product_embeddings_df_len = len(product_embeddings_df)

    for index, embedding in product_embeddings_df.iterrows():

        productName_embedding_array = np.array(
                                        embedding["productName_embedding"].tolist()
                                        ).reshape(1, -1)
        
        brandName_embedding_array = np.array(
                                        embedding["brandName_embedding"].tolist()
                                         ).reshape(1, -1)

        product_name_similarity = cosine_similarity(
            user_reuest_product_name_embedding_array, productName_embedding_array
        )[0][0]

        brand_name_similarity = cosine_similarity(
            user_reuest_brand_name_embedding_array, brandName_embedding_array
        )[0][0]

        # print("product_name_similarity:", product_name_similarity)
        # print("brand_name_similarity:", brand_name_similarity)

        overall_similarity = (
              product_name_similarity * product_name_weight
            + brand_name_similarity * brand_name_weight
        )

        if overall_similarity > highest_similarity:
            highest_similarity = overall_similarity
            best_matched_productID = embedding["productID"]
...
def create_order():
    ...
    model_name_roberta = "roberta-base-nli-stsb-mean-tokens"
    model_name_distilbert = "distilbert-base-nli-stsb-mean-tokens"
    model_name_minilm_l6 = "all-MiniLM-L6-v2"
    top_n = 20
    file_open_mode = "rb"

    # Open pickle files
    # brand_name_embeddings_file_obj = open_pickle_files(brand_name_embeddings_file_path, file_open_mode)
    product_embeddings_file_obj = open_pickle_files(product_embeddings_file_path, file_open_mode)
    ...
    if product_embeddings_file_obj is None:
        ...
        # model_name = model_name_minilm_l6
